...
module.exports = function (ctx) {
  ...
  if (ctx.hook == "after_plugin_install") {
    console.log(
      "Adding required keychain sharing capability (ADALiOS.entitlements).."
    );
    action = ACTION_INSTALL;
  } else if (
    ctx.hook == "before_plugin_uninstall" ||
    ctx.hook == "before_plugin_rm"
  ) {
    action = ACTION_UNINSTALL;
    console.log(
      "Removing keychain sharing capability (ADALiOS.entitlements).."
    );
  } else {
  ...
  try {
    ...
    iosProjectFile = require(path.join(
  ...
  fs.readdir(platformRoot, function (err, data) {
    ...
    if (!projFolder) {
      throw new Error(
        "Could not find an .xcodeproj folder in: " + platformRoot
      );
    }
    ...
    var entitlementsFile = path.join(
      projName,
      "Resources/ADALiOS.entitlements"
    );
    ...
    if (action == ACTION_INSTALL) {
      ...
      // uninstall
      console.log(
        "Removing entitlements from " + CODE_SIGN_ENTITLEMENTS + " section"
      );
